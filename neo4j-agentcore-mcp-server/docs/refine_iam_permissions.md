...
This document describes how to separate IAM permissions for three personas when working with the Neo4j MCP Server (`neo4j-agentcore-mcp-server/`) and AgentCore Agents (`neo4j-agentcore-agents/`):
...
### Agent Stack (`neo4j-agentcore-agents/`)
...
The CloudFormation template (`neo4j-agentcore-agents/cfn/agent-runtime.yaml`) requires:
...
aws bedrock-agentcore-control delete-agent-runtime \
  --agent-runtime-id test-id --region us-east-1
