...
IAM permissions for running the LangGraph agents and notebooks with AWS Bedrock in SageMaker Studio or other AWS environments.

## Table of Contents

- [Quick Start](#quick-start)
- [Tooling Permissions (script and notebooks)](#tooling-permissions-script-and-notebooks)
...
## Tooling Permissions (script and notebooks)

`inference-profiles/setup-inference-profile.sh` and the notebooks manage and use Bedrock inference profiles. They do not create any IAM roles.
...
| `bedrock:DeleteInferenceProfile` | Delete profiles (`--delete`) |
| `bedrock:ListTagsForResource` | Check tags on profiles |
| `bedrock-runtime:Converse` | Test profiles (`--test`) |

### Notebooks

| Notebook | Permissions |
|----------|-------------|
| `notebooks/minimal_langgraph_agent.ipynb` | `bedrock-runtime:Converse` |
| `notebooks/neo4j_simple_mcp_agent.ipynb` | `bedrock-runtime:Converse` |
| `notebooks/neo4j_strands_mcp_agent.ipynb` | `bedrock-runtime:Converse`, `bedrock-runtime:InvokeModel` |

The MCP Gateway notebooks (`neo4j_simple_mcp_agent.ipynb` and `neo4j_strands_mcp_agent.ipynb`) authenticate to the gateway with a Bearer token over HTTPS, not IAM. The gateway credentials come from `.mcp-credentials.json`.

### Combined policy for the script and all notebooks
...
./inference-profiles/setup-inference-profile.sh

# Create with specific model
./inference-profiles/setup-inference-profile.sh sonnet      # Claude 3.5 Sonnet
./inference-profiles/setup-inference-profile.sh sonnet-4.5  # Claude Sonnet 4.5

# List existing profiles
./inference-profiles/setup-inference-profile.sh --list

# Delete profile
./inference-profiles/setup-inference-profile.sh --delete
```
