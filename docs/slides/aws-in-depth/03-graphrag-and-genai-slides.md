...
How knowledge graphs fix retrieval, from traditional RAG's limits to GraphRAG. Background on GenAI limits and embedding fundamentals is in the appendix.

---

## The RAG Retrieval Flow

Traditional RAG retrieves the chunks most similar to a question and hands them to the LLM. (See the appendix for embedding and vector-search fundamentals.)
...
GraphRAG still enters the graph through vector search, so it starts with a vector index.
...
GenAI limits, traditional RAG, and embedding fundamentals. Background for the GraphRAG story above.
