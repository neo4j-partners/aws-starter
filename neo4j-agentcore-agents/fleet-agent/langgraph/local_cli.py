...
# Anchored to the agent root (two levels up from this variant file) so the
# credentials file resolves no matter the working directory.
CREDENTIALS_FILE = Path(__file__).resolve().parent.parent / ".mcp-credentials.json"
...
def load_credentials() -> dict:
    """Load credentials from .mcp-credentials.json (static, no refresh)."""
