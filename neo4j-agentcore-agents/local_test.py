...
Usage:
    ...
    uv run local-test build orchestrator-agent
    uv run local-test run orchestrator-agent
    uv run local-test test orchestrator-agent
...
@app.command()
def build(
    agent: AgentType = typer.Argument(..., help="Agent to build"),
    ...
    # Agents split into framework variants keep their Dockerfile in the
    # variant subdir; others (e.g. orchestrator-agent) keep it at the root.
    variant_dockerfile = agent_dir / variant / "Dockerfile"
    if variant_dockerfile.exists():
        dockerfile = variant_dockerfile
        dockerfile_arg = f"{variant}/Dockerfile"
        console.print(
            f"\n[bold blue]Building Docker Image: {agent_name} ({variant})[/bold blue]\n"
        )
    else:
...
@app.command()
def run(
    agent: AgentType = typer.Argument(..., help="Agent to run"),
...
@app.command()
def stop(
    agent: AgentType = typer.Argument(..., help="Agent to stop"),
):
...
@app.command()
def test(
    agent: AgentType = typer.Argument(..., help="Agent to test"),
