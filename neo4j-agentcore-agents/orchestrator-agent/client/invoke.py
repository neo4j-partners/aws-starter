...
Usage:
    uv run orchestrator-invoke                          # Uses default prompt
    uv run orchestrator-invoke "What is the schema?"    # Custom prompt
    uv run orchestrator-invoke load-test                # Load test mode (random queries every 5s)
    uv run orchestrator-invoke load-test --interval 10  # Custom interval in seconds
...
def get_agent_config() -> tuple[str, str]:
    ...
    """
    # Anchor to the agent root (parent of the `client` package), not the
    # current working directory, so this resolves from any cwd.
    config_file = Path(__file__).resolve().parent.parent / ".bedrock_agentcore.yaml"
