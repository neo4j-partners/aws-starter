...
Uses credentials from .mcp-credentials.json (generated by ./deploy.py credentials).
...
Set MCP_ENV=NAME to read .mcp-credentials.NAME.json instead, matching the
--env NAME selector on ./deploy.py and ./cloud.sh.

Prerequisites:
    Run './deploy.py credentials' to generate .mcp-credentials.json
"""
...
# MCP_ENV carries the --env suffix chosen on the command line, so the
# credentials read here always belong to the .env that produced them.
ENV_SUFFIX = os.environ.get("MCP_ENV", "")
CREDENTIALS_FILE = SERVER_DIR / (
    f".mcp-credentials.{ENV_SUFFIX}.json" if ENV_SUFFIX else ".mcp-credentials.json"
)


def load_credentials() -> dict:
    """Load credentials from the credentials file for the selected deployment."""
    if not CREDENTIALS_FILE.exists():
        print(f"ERROR Credentials file not found: {CREDENTIALS_FILE.name}")
        print()
        print(f"Run './deploy.py{_env_flag()} credentials' to generate it")
        sys.exit(1)
...
def _env_flag() -> str:
    """The `--env NAME` fragment to echo back in messages, if one is in use."""
    return f" --env {ENV_SUFFIX}" if ENV_SUFFIX else ""
...
def get_credentials() -> dict:
    ...
    if not check_token_expiry(credentials):
        ...
        print()
        print(f"Run './deploy.py{_env_flag()} credentials' to refresh")
        sys.exit(1)
...
def cmd_token():
    ...
    if check_token_expiry(credentials):
        ...
        print(
            "Status: EXPIRED - run "
            f"'./deploy.py{_env_flag()} credentials' to refresh"
        )
        sys.exit(1)
...
async def cmd_mcp_operation(operation, operation_name: str):
    ...
    print(f"INFO  Loading credentials from {CREDENTIALS_FILE.name}...")
...
def show_help():
    """Show help message."""
    print(f"""Neo4j MCP Gateway Client

Target: AgentCore Gateway
Auth:   Uses {CREDENTIALS_FILE.name} (generated by ./deploy.py credentials)
...
Prerequisites:
  Run './deploy.py{_env_flag()} credentials' to generate {CREDENTIALS_FILE.name}

Multiple deployments:
  MCP_ENV=NAME selects .mcp-credentials.NAME.json. Prefer './cloud.sh --env
