...
# Generated .env files are written next to this script, regardless of cwd.
...
get_model_info() {
    ...
    case "$model_key" in
        haiku|haiku45)
            echo "global.anthropic.claude-haiku-4-5-20251001-v1:0|Claude Haiku 4.5|fast and cheap"
            ;;
        sonnet|sonnet45)
            echo "global.anthropic.claude-sonnet-4-5-20250929-v1:0|Claude Sonnet 4.5|most capable"
            ;;
        *)
            echo ""
            ;;
...
show_help() {
    ...
    echo "  haiku, haiku45     Claude Haiku 4.5   (fast, cheap - good for testing)"
    echo "  sonnet, sonnet45   Claude Sonnet 4.5  (most capable)"
    echo ""
...
create_profile() {
    ...
    # Without DataZone IDs the profile cannot get the AmazonBedrockManaged tag,
    # so it will be rejected inside SageMaker Unified Studio. Refuse to create a
    # silently-broken profile unless the caller explicitly opts in (non-SMUS
    # use such as EC2/local/cost-tracking).
    if [ -z "$DATAZONE_PROJECT_ID" ] || [ -z "$DATAZONE_DOMAIN_ID" ]; then
        if [ "${ALLOW_UNTAGGED_PROFILE:-}" != "1" ]; then
            echo ""
            echo -e "${RED}Error: DataZone IDs not detected.${NC}"
            echo "  A profile created now would lack the AmazonBedrockManaged=true tag"
            echo "  and would be blocked inside SageMaker Unified Studio."
            echo ""
            echo "  Fix: run '$0 --detect' and ensure a SageMaker Unified Studio"
            echo "       domain/project exists in ${REGION}."
            echo "  Override (non-SMUS use only): ALLOW_UNTAGGED_PROFILE=1 $0 ${model_key}"
            exit 1
        fi
        echo -e "${YELLOW}⚠ ALLOW_UNTAGGED_PROFILE=1: creating an untagged profile (will not work in SMUS)${NC}"
    fi
