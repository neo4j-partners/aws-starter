...
def send_cfn_response(event: dict, status: str, reason: str = None,
    ...
    response_body = {
        ...
        'PhysicalResourceId': physical_resource_id or event.get('PhysicalResourceId', 'health-check'),
    ...
    for attempt in range(1, MAX_POLL_ATTEMPTS + 1):
        try:
            ...
            if status in ready_states:
                logger.info(f'Runtime is {status}, waiting {WARMUP_WAIT_SECONDS}s for container warmup...')
