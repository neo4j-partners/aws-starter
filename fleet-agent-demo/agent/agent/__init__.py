...
Importing this package loads the shared ``.env`` at the fleet-agent-demo
root (two levels up from ``agent/``), so local runs pick up the Neo4j
...
# The shared .env at the fleet-agent-demo root is the local config source.
