...
directory). The access token is refreshed in memory only — no file writes —
so this works unchanged on AgentCore Runtime where the filesystem is
ephemeral.
"""
    ...
    if _CREDENTIALS is None:
        if not CREDENTIALS_FILE.exists():
            raise FileNotFoundError(
                ...
                "Copy from the MCP server deployment or a sibling agent:\n"
                "  cp ../fleet-agent/.mcp-credentials.json ."
            )
