...
[fleet-agent](../neo4j-agentcore-agents/fleet-agent/) and the
...
2. **The fleet-agent adapts automatically.** It fetches the schema at runtime
   ...
   with no changes. Its retriever embedder must match the one this pipeline
   used (default: Bedrock Titan v2, 1024 dims). Try the queries in
